...
public class Group extends PersistedObject implements Comparable<Group> {
	...
	// Considers that spawn claims should be worth 5 (== the value set in config.yml)
	public int getWeightedClaimCount() {
          return claims.getList().size() + (StandardPlugin.getPlugin().getSpawnClaimCost() - 1) * spawnClaims.getValue();
	}
